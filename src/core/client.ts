...
class TuyaOpenApiClient {
  ...
  async request<T>({
    ...
    headers = {},
    retry = true,
  }: TuyaOpenApiClientRequestOptions): Promise<
    AxiosResponse<TuyaOpenApiResponse<T>>
  > {
    const t = Date.now().toString();
    ...
    if (this.version === "v2") {
      const { path: encodePath, ...signHeaders } = await this.getSignHeaders(
        path,
        method,
        query,
        body
  ...
  async getSignHeaders(
    path: string,
    method: string,
    query: TuyaOpenApiClientRequestQueryBase,
    body: TuyaOpenApiClientRequestBodyBase
  ): Promise<TuyaOpenApiClientRequestExtHeader> {
    const t = Date.now().toString();
  ...
  async refreshSignV2(
    t: string,
    headers: TuyaOpenApiClientRequestHeaderBase
  ): Promise<{ sign: string; signHeaders: string }> {
    const nonce = "";
    const method = "GET";
  ...
  async getHeaderV2(
    ...
    const { sign, signHeaders } = forRefresh
